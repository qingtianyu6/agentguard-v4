...
此次代码修复：严格模式未知 tool/action 的 preflight DENY，ToolHive webhook 只接受与动作匹配的已审核活动契约；后端 49 passed。仍须部署和独立测试，不能将单元测试结果外推为 M1 Gate 通过。开源依赖及 ToolHive 接口与配置已按第三方清单接入；未发布至用户 GitHub，尚缺目标仓库及授权。
